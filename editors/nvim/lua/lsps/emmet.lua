...
local emmet_ls = {
    ...
    config = {
        cmd = { "$LSP_BIN", "--stdio" },
        filetypes = { "html", "css", "scss", "less", "javascript", "typescript", "jsx", "tsx", "svelte" },
    },
}

---@return string The path that contains the language server
local function get_editor_lsp_dir()
    return string.format("%s/miversen/lsps/emmet", vim.fn.stdpath("data"))
end
...
function emmet_ls.get_binary_path(ignore)
    local editor_lsp = string.format("%s/emmet-ls/node_modules/.bin/emmet-ls", get_editor_lsp_dir())
...
function emmet_ls.needs_install()
    return vim.fn.filereadable(emmet_ls.get_binary_path()) ~= 1
end

-- Downloads and installs the emmet language server
...
function emmet_ls.install(success_callback, error_callback, install_opts)
    install_opts = install_opts or { force = false }

    local target_path = string.format("%s/emmet-ls", get_editor_lsp_dir())
    local temp_dir = string.format("%s/neovim_miversen_lsp_download-emmet-%s",
        vim.uv.os_tmpdir(), os.date("%Y%m%d%H%M%S"))

    -- Skip installation if already exists and not forced
    if not install_opts.force and emmet_ls.get_binary_path() ~= "MISSING BINARY" then
        success_callback()
        return
    end
    ...
    local function install_from_npm(version)
        vim.notify("Installing Emmet Language Server from npm", vim.log.levels.DEBUG)

        local package_spec = string.format("emmet-ls@%s", version)
        ...
        local complete = function(result)
            if result.exit_code ~= 0 then
                cleanup()
                error_callback("Failed to install emmet-ls: " .. (result.stderr or ""),
                    result.exit_code)
                return
            end

            -- Check if the Emmet language server was installed
            local server_path = string.format(
                "%s/node_modules/.bin/emmet-ls", temp_dir)
            if vim.fn.filereadable(server_path) == 0 then
                cleanup()
                error_callback("Emmet language server binary not found after npm install", -1)
                return
            end
            ...
            vim.notify("Emmet Language Server Installation Complete", vim.log.levels.INFO)
            success_callback()
        end
    ...
    if install_opts.version then
        ...
        emmet_ls.latest_version(function(version)
            if not version then
                cleanup()
                error_callback("Unable to determine latest Emmet language server version", -1)
                return
            end
...
--Checks the current emmet language server version and returns it
--NOTE: Will return -1 if it cannot find the lsp
---@return string
function emmet_ls.version()
    if emmet_ls.needs_install() then
        emmet_ls._current_version = nil
        return "-1"
    end
    if emmet_ls._current_version then
        -- Lets used the cached version
        return emmet_ls._current_version
    end

    local package_json = string.format("%s/emmet-ls/node_modules/emmet-ls/package.json",
        get_editor_lsp_dir())
    ...
    if not version then
        ...
        emmet_ls._current_version = version
        return version
    end
end

---@param callback fun(version: string?) If provided, we will provide the latest version (or nothing if we can't find it)
function emmet_ls.latest_version(callback)
    if emmet_ls._latest_version then
        callback(emmet_ls._latest_version)
        return
    end

    local npm_api = "https://registry.npmjs.org/emmet-ls/latest"

    ---@param result Shell.Serial
    local complete = function(result)
        if result.exit_code ~= 0 or result.stdout:len() == 0 then
            emmet_ls._latest_version = nil
            callback()
            return
        end
        ...
        emmet_ls._latest_version = version
        callback(version)
    end
...
function emmet_ls.needs_update(callback)
    -- If we aren't installed, we can't update
    if emmet_ls.needs_install() then
        callback(false)
        return
    end
    local current_version = emmet_ls.version()
    ...
    emmet_ls.latest_version(complete)
    return
end
