...
local function get_editor_lsp_dir()
    return string.format("%s/miversen/lsps/js", vim.fn.stdpath("data"))
end
...
function tsserver.get_binary_path(ignore)
    local editor_lsp = string.format("%s/tsserver/node_modules/typescript-language-server/lib/cli.mjs",
        get_editor_lsp_dir())
...
function tsserver.needs_install()
    return vim.fn.filereadable(tsserver.get_binary_path()) ~= 1
end

-- Downloads and installs the typescript language server
...
function tsserver.install(success_callback, error_callback, install_opts)
    install_opts = install_opts or { force = false }

    local target_path = string.format("%s/tsserver", get_editor_lsp_dir())
    local temp_dir = string.format("%s/neovim_miversen_lsp_download-js-%s",
        vim.uv.os_tmpdir(), os.date("%Y%m%d%H%M%S"))

    -- Skip installation if already exists and not forced
    if not install_opts.force and tsserver.get_binary_path() ~= "MISSING BINARY" then
        success_callback()
        return
    end
    ...
    local function install_from_npm(version)
        vim.notify("Installing TypeScript Language Server from npm", vim.log.levels.DEBUG)

        local package_spec = string.format("typescript-language-server@%s typescript", version)
        ...
        local complete = function(result)
            if result.exit_code ~= 0 then
                cleanup()
                error_callback("Failed to install typescript-language-server: " .. (result.stderr or ""),
                    result.exit_code)
                return
            end

            -- Check if the TypeScript language server was installed
            local server_path = string.format(
                "%s/node_modules/typescript-language-server/lib/cli.mjs", temp_dir)
            if vim.fn.filereadable(server_path) == 0 then
                cleanup()
                error_callback("TypeScript language server binary not found after npm install", -1)
                return
            end
            ...
            vim.notify("TypeScript Language Server Installation Complete", vim.log.levels.INFO)
            success_callback()
        end
    ...
    if install_opts.version then
        ...
        tsserver.latest_version(function(version)
            if not version then
                cleanup()
                error_callback("Unable to determine latest TypeScript language server version", -1)
                return
            end
...
--Checks the current tsserver version and returns it
--NOTE: Will return -1 if it cannot find the lsp
---@return string
function tsserver.version()
    if tsserver.needs_install() then
        tsserver._current_version = nil
        return "-1"
    end
    if tsserver._current_version then
        -- Lets used the cached version
        return tsserver._current_version
    end

    local package_json = string.format("%s/tsserver/node_modules/typescript-language-server/package.json",
        get_editor_lsp_dir())
    ...
    if not version then
        ...
        tsserver._current_version = version
        return version
    end
end

---@param callback fun(version: string?) If provided, we will provide the latest version (or nothing if we can't find it)
function tsserver.latest_version(callback)
    if tsserver._latest_version then
        callback(tsserver._latest_version)
        return
    end

    local npm_api = "https://registry.npmjs.org/typescript-language-server/latest"

    ---@param result Shell.Serial
    local complete = function(result)
        if result.exit_code ~= 0 or result.stdout:len() == 0 then
            tsserver._latest_version = nil
            callback()
            return
        end
        ...
        tsserver._latest_version = version
        callback(version)
    end
...
function tsserver.needs_update(callback)
    -- If we aren't installed, we can't update
    if tsserver.needs_install() then
        callback(false)
        return
    end
    local current_version = tsserver.version()
    ...
    tsserver.latest_version(complete)
    return
end
