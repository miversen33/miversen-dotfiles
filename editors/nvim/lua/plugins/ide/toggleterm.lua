...
local terms = {}
local termopts = {
    float = {cmd = "$SHELL", direction = "float", close_on_exit = false},
    right = {
        cmd = "$SHELL",
        direction = "vertical",
        close_on_exit = false,
...
local function get_term(termtype)
    if terms[termtype] then return terms[termtype] end
...
local function register_user_funcs()
    vim.api.nvim_create_user_command('CFloatTerm',
                                     function() get_term('float'):toggle() end,
                                     {})
    vim.api.nvim_create_user_command('CSplitTerm', function(args)
        ...
        elseif args.fargs[1] == 'horizontal' then
            ...
            vim.notify(
                string.format("I have no idea what direction `%s` is...",
                              direction), vim.log.levels.WARN)
            return
        end
...
local toggleterm_opts = {
    ...
    float_opts = {
        ...
        width = function() return c_width(.90) end,
        height = function() return c_height(.90) end
    }
}
