-- Yoined straight from netman source: https://github.com/miversen33/netman.nvim/blob/main/lua/netman/tools/shell.lua
...
---@class Shell.Serial
---@field command string The command that was ran
---@field cmd_pieces string[] The command (as a table) that was passed to Shell.new
---@field signal integer? The signal provided at the end of the process if there was one
---@field stdout string[] The contents of standard out for this process. May be empty if run wasn't called or standard out was cleared during pipe read
---@field stderr string[] The contents of standard error for this process. May be empty if run wasn't called or standard error was cleared during pipe read
---@field exit_code integer The code that the process exited with
---@field pid integer The pid of the process. Only really useful for asynchronous processes that are not yet complete
---@field opts Shell.Flag[] The options that were provided to create the shell

---@class Shell.Flag
---@field STDOUT_JOIN string? A character to join standard output on
---@field STDERR_JOIN string? A character to join standard error on
---@field STDOUT_FILE string? A file to dump standard output lines to
---@field STDERR_FILE string? A file to dump standard error lines to
---@field STDOUT_FILE_IS_BINARY boolean? Indicates that the STDOUT_FILE is a binary file. Assumption is that it's text
---@field STDERR_FILE_IS_BINARY boolean? Indicates that the STDERR_FILE is a binary file. Assumption is that it't text
---@field STDOUT_FILE_OVERWRITE boolean? Indicates that we should overwrite the contents of STDOUT_FILE. Assumption is to append
---@field STDERR_FILE_OVERWRITE boolean? Indicates that we should overwrite the contents of STDERR_FILE. Assumption is to append
---@field ASYNC boolean? Indicates that you want this process to run asynchronously. Assumption is synchronous
---@field EXIT_CALLBACK fun(shell: Shell.Serial)? A function that is called when the job is complete. You can also call shell.add_exit_callback
---@field STDOUT_CALLBACK fun(stdout: string)? A function that is called when the job has output to provide. You can also call shell.add_stdout_callback
---@field STDERR_CALLBACK fun(stderr: string)? A function that is called when the job has an error to provide. You can also call shell.add_stderr_callback
---@field SIGNAL_CALLBACK fun(signal: integer): boolean? A function that is called when the job receives a signal. Return true to indicate you consumed it, or false/nil to indicate we need to process it
---@field STDOUT_PIPE_LIMIT integer? The amount of lines to allow the STDOUT table to grow to. Unset by default (meaning the pipe is unlimited). Set to 0 if you are using STDOUT_CALLBACK
---@field STDERR_PIPE_LIMIT integer? The amount of lines to allow the STDERR table to grow to. Unset by default (meaning the pipe is unlimited). Set to 0 if you are using STDERR_CALLBACK
---@field ENV table<string, string>? Any environment variables you wish to provide to the job. Starts with a fresh environment (IE, empty table)
---@field UID integer? The user id to run the job as. Note, you will not be able to conditionally grant permission to change UID. IE, you likely need to be root to use this
---@field GID integer? The group id to run the job as. Note, you will not be able to conditionally grant permission to change GID. IE, you likely need to be root to use this
---@field DETACHED boolean? Detaches the process from the parent. See https://github.com/luvit/luv/blob/master/docs.md#uvspawnpath-options-on_exit for details
...
--- @param command string[] Array (table that is not key,value pairs) of commands to run
